...
#if defined(NDEBUG)
# error "Parrotcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf600f860be117213fb66a2a4fa6b31eb84230a0cade35a2a65bbae2a12d49390");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Parrotcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Parrotcoin Signed Message:\n";
...
{
    // Parrotcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Parrotcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 30; // Parrotcoin: 3.5 days
static const int64 nTargetSpacing = 1 * 30; // Parrotcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Parrotcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Parrotcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf600f860be117213fb66a2a4fa6b31eb84230a0cade35a2a65bbae2a12d49390");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "I create a new world!";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x440c4d440f0300033d49394b2456339e58de8f7c7a2a2d892c117078a0f4771e"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Parrotcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ParrotcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ParrotcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ParrotcoinMiner(CWallet *pwallet)
{
    printf("ParrotcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("parrotcoin-miner");
    ...
    try { loop {
        ...
        printf("Running ParrotcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ParrotcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ParrotcoinMiner, pwallet));
}
